...
import com.acelych.atoz.data.DataHandler;
import com.acelych.atoz.data.FileHandler;
import com.acelych.atoz.data.MediaHandler;
import com.acelych.atoz.data.Word;
import com.acelych.atoz.ui.ComboBoxUI;
import com.acelych.atoz.ui.ScrollBarUI;
import com.acelych.atoz.ui.SliderUI;
import com.acelych.atoz.ui.TextFieldUI;
import com.acelych.atoz.widget.DialogFactory;
import com.acelych.atoz.widget.GlassPanel;
import com.acelych.atoz.widget.MainTable;
import com.acelych.atoz.widget.SeparatorFactory;
...
public class MainFrame extends JFrame
{
    ...
    private MainTable wordListTable;
    ...
    private DialogFactory.ConfirmationDialog dialogOnNotEnoughWords, dialogOnAddWordSucceed, dialogSyncFailed;
    private DialogFactory.InformationDialog dialogWordInfo;
    ...
    MainFrame(String string)
    {
        ...
        dialogSyncFailed = DialogFactory.createConfirmationDialog(this, rightPanel, "Word Sync Failed !");
        dialogWordInfo = DialogFactory.createInformationDialog(this, rightPanel, "");
    ...
    private void initFile()
    {
        ...
        wordListTable = new MainTable(wordListModel);
        ...
        //wordListTable
        wordListTable.getTableHeader().setDefaultRenderer(new HeaderRenderer());
        wordListTable.getColumnModel().getColumn(0).setCellRenderer(new RegularRenderer(consolas01));
        wordListTable.getColumnModel().getColumn(1).setCellRenderer(new RegularRenderer(yahei01));
        wordListTable.getColumnModel().getColumn(2).setCellRenderer(new RegularRenderer(consolas01));
        wordListTable.getColumnModel().getColumn(3).setCellRenderer(new ProficiencyRenderer());
        wordListTable.getColumnModel().getColumn(4).setCellRenderer(new BooleanRenderer());
        wordListTable.getColumnModel().getColumn(5).setCellRenderer(new BooleanRenderer());
        wordListTable.addKeyListener(new KeyAdapter()
        {
            @Override
        ...
        wordListTable.addMouseListener(new MouseAdapter()
        {
            @Override
            public void mouseClicked(MouseEvent e)
            {
                ...
                {
                    String wordName = wordListTable.getValueAt(wordListTable.getSelectedRow(), 0).toString();
                    dialogWordInfo.dialogTitle.setText(wordName.substring(0, 1).toUpperCase() + wordName.substring(1));

                    List<String> crawlerResult = CrawlerHandler.getDetailedTranslation(wordName);
                    if (crawlerResult != null)
                    {
                        StringBuilder transResult = new StringBuilder("<html>");
                        FontMetrics fontMetrics = dialogWordInfo.dialogTrans.getFontMetrics(dialogWordInfo.dialogTrans.getFont());
                        for (int i = 3; i < crawlerResult.size(); i++)
                        {
                            String loopStr = crawlerResult.get(i);
                            char[] chars = loopStr.toCharArray();
                            for (int start = 0, len = 0; start + len < loopStr.length();)
                            {
                                while (true)
                                {
                                    len++;
                                    if (start + len > loopStr.length()) // inner loop reach outer's target
                                        break;
                                    if (fontMetrics.charsWidth(chars, start, len) - fontMetrics.stringWidth("<b></b>")
                                            > dialogWordInfo.dialogTrans.getWidth()) // inner loop reach inner's target
                                        break;
                                }
                                transResult.append(chars, start, len - 1).append("<br/>");
                                start += len - 1; len = 0;
                            }
                            if (i + 1 == crawlerResult.size()) //html end tag
                                transResult.append("</html>");
                        }

                        dialogWordInfo.dialogRange.setText(crawlerResult.get(0));
                        dialogWordInfo.dialogPhonetic.setText(crawlerResult.get(1).length() < 16 ?
                                crawlerResult.get(1)+" , "+crawlerResult.get(2) :
                                "<html>" + crawlerResult.get(1) + "<br/>" + crawlerResult.get(2) + "</html>");
                        dialogWordInfo.dialogTrans.setText(transResult.toString());
                        dialogWordInfo.dialogWarning.setVisible(false);
                    }
                    else
                    {
                        dialogWordInfo.dialogRange.setText("");
                        dialogWordInfo.dialogPhonetic.setText("");
                        dialogWordInfo.dialogTrans.setText("");
                        dialogWordInfo.dialogWarning.setVisible(true);
                    }
                    dialogWordInfo.setVisible(true);
                }
            }
        });
    }
    ...
    private void initRightPanelAddWord()
    {
        ...
        JButton buttonSync = new JButton(new ImageIcon(MediaHandler.imageSync));
        ...
        JLabel separator01 = SeparatorFactory.builder(SeparatorFactory.SHORT);
        JLabel separator02 = SeparatorFactory.builder(SeparatorFactory.SHORT);
        ...
        buttonSync.setFocusPainted(false);
        buttonSync.setPressedIcon(new ImageIcon(MediaHandler.imageSyncHL));
        buttonSync.addActionListener(e ->
        {
            if (textFieldName.getText().equals("") || textFieldName.getText() == null)
            {
                nameExclamation.setVisible(true);
                transExclamation.setVisible(false);
                return;
            }
            Word word = CrawlerHandler.generateWord(textFieldName.getText());
            if (word == null)
            {
                dialogSyncFailed.setVisible(true);
                return;
            }
            textFieldTrans.setText(word.translatedName);
            checkBoxIsCET4.setSelected(word.isCET4);
            checkBoxIsCET6.setSelected(word.isCET6);
        ...
        rightPanelAddWord.add(separator01);
        rightPanelAddWord.add(separator02);
        rightPanelAddWord.add(buttonContinue);
        rightPanelAddWord.add(buttonSync);
        ...
        SpringLayout.Constraints separatorC = addWordLayout.getConstraints(separator01);
        ...
        addWordLayout.putConstraint(SpringLayout.NORTH, separator02, 0, SpringLayout.NORTH, checkBoxIsCET6);
        addWordLayout.putConstraint(SpringLayout.WEST, separator02, 3, SpringLayout.EAST, checkBoxIsCET6);
        addWordLayout.putConstraint(SpringLayout.VERTICAL_CENTER, buttonSync, 0, SpringLayout.VERTICAL_CENTER, checkBoxIsCET6);
        addWordLayout.putConstraint(SpringLayout.WEST, buttonSync, -8, SpringLayout.EAST, separator02);
    }
    ...
    private void initRightPanelWordList()
    {
        ...
        //---ScrollPane Part---//
        JScrollPane wordListScroll = new JScrollPane(this.wordListTable, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,JScrollPane.HORIZONTAL_SCROLLBAR_AS_NEEDED);
...
//            System.out.println("Name: "+temp.name+", Score: "+temp.score);
    ...
    private void onDeleteKeyReleased()
    {
        ...
        {
            int rowNum = wordListTable.getSelectedRow();
